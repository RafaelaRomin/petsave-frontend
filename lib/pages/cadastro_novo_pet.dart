...
class CadastroNovoPet extends StatefulWidget {
  const CadastroNovoPet({
    Key? key,
  }) : super(key: key);

  @override
  State<CadastroNovoPet> createState() => _CadastroNovoPetState();
}

class _CadastroNovoPetState extends State<CadastroNovoPet> {
  ...
  Future<void> cadastrarPet() async {
    if (userToken == null || userId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Você precisa estar logado para cadastrar um pet.')),
      );
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        showDialog(
          context: context,
          barrierDismissible:
              false, // Impede que o usuário feche o diálogo clicando fora dele
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Erro ao cadastrar pet: ${response.statusCode} - ${response.body}')),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
